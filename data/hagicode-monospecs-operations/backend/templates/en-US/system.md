...
Every operation must stay within the selected project root `{{projectPath}}`. Repositories and other references in `{{targetScopeMarkdown}}` are read-only: never create, modify, or delete files there. Do not commit, push, or publish.
