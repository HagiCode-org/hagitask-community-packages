#!/usr/bin/env node
/**
 * HagiTask Community package validation entrypoint.
 *
 ...
 * or falls back to the Community repository root.
 *
...
/**
 * Validate all community packages under `repoRoot`.
 * @param {string} repoRoot
 * @returns {{ packages: string[], errors: ValidationError[] }}
 */
export function validateCommunityPackages(repoRoot) {
...
function main() {
  ...
  const summaryLines = [];
  summaryLines.push(`## Community package validation`);
  ...
  if (errors.length === 0) {
    console.log(`✓ Community validation passed for ${packages.length} package(s): ${packages.join(', ')}`);
  ...
  console.error(`✗ Community validation failed with ${errors.length} error(s):`);
